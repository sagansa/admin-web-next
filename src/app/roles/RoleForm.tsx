...
export default function RoleForm({ role, onClose }: RoleFormProps) {
  ...
  const getPermissionDescription = (name: string) => {
    const descriptions: Record<string, string> = {
      'access-backoffice': 'Allows access to the Admin Web Dashboard',
      'access-pos': 'Allows access to the POS application',
      'access-attendance': 'Allows access to the Presence/Attendance application',
    };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ general: getErrorMessage(error, 'Failed to save role') });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-h-[90vh] overflow-auto">
      <div className="flex items-center justify-between border-b pb-3 sticky top-0 bg-white z-10">
        <h3 className="text-lg font-medium text-gray-900">{role ? 'Edit Role' : 'Create Role'}</h3>
        <button
          type="button"
          onClick={onClose}
